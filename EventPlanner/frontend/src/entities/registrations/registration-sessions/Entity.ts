import type { Entity as Session } from "@/entities/sessions"

// A join row linking Registration to Session (back-end `e.Related(x => x.SelectedSessions)`). Rows are picked
...
// the full model class instead is what makes `add({ sessionId, session })` fail to type-check.
//
