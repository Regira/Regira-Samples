<template>
    <form @submit.prevent="handleSubmit">
        ...
        <div v-if="item.imageUrl" class="venue-banner mb-3" :style="{ backgroundImage: `url(${item.imageUrl})` }" />
        ...
        <FormSection :title="$t(config.detailsTitle || '')" :readonly="readonly">
            ...
            <div class="row">
                <div class="col-md-6 mb-2">
                    <input v-model="item.address" :readonly="readonly" class="form-control" required maxlength="256" />
                    <FormLabel :label="$t('address')" />
                </div>
                <div class="col-md-3 mb-2">
                    <input v-model="item.city" :readonly="readonly" class="form-control" required maxlength="128" />
                    <FormLabel :label="$t('city')" />
                </div>
                <div class="col-md-3 mb-2">
                    <input v-model="item.postalCode" :readonly="readonly" class="form-control" maxlength="32" />
                    <FormLabel :label="$t('postalCode')" />
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 mb-2">
                    <input v-model="item.country" :readonly="readonly" class="form-control" required maxlength="128" />
                    <FormLabel :label="$t('country')" />
                </div>
                <div class="col-md-6 mb-2">
                    <input v-model="item.imageUrl" :readonly="readonly" class="form-control" placeholder="https://…" maxlength="1024" />
...
.venue-banner {
    ...
    box-shadow: inset 0 -40px 40px -10px rgba(0, 0, 0, 0.35);
}
</style>
