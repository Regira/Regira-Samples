<template>
    <div class="row border-bottom py-2 align-items-center">
        ...
        <div class="col text-truncate d-flex align-items-center gap-2">
            <span class="avatar-sm rounded-circle" :style="item.avatarUrl ? { backgroundImage: `url(${item.avatarUrl})` } : undefined" />
            {{ item.$title }}
        </div>
        <div class="col d-none d-md-block text-truncate">{{ item.email }}</div>
        <div class="col d-none d-lg-block text-truncate">{{ item.department }}</div>
        <div class="col d-none d-xl-block text-truncate">{{ item.jobTitle }}</div>
