...
const api = "/event-categories"

const config: IConfig = {
    id: Entity.name,
    key: "EventCategory", // TODO: route-name prefix + icon key (conventionally = Entity.name)
    isComplex: false, // flat lookup — title, colour, icon; no relations → modal form (FormModalButton)

    routePrefix: "event-categories", // TODO: URL path segment
    ...
    overviewTitle: "eventCategories", // camelCase i18n keys (multi-word → e.g. shoppingLists / shoppingList) — add matching entries to public/data/translations.json, or the nav renders the raw key
    detailsTitle: "eventCategory",
