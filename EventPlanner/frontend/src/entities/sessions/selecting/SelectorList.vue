<template>
    <div class="entity-list">
        <div class="row fw-bold border-bottom pb-2">
            ...
            <div class="col d-none d-md-block">{{ $t("startTime") }}</div>
            <div class="col d-none d-lg-block">{{ $t("room") }}</div>
        </div>
        <div v-for="item in items" :key="item.$id" class="row border-bottom py-2" :class="{ 'is-selected': isSelected(item) }">
            ...
            <div class="col d-none d-md-block text-truncate">{{ formatDateTime(item.startTime) }}</div>
            <div class="col d-none d-lg-block text-truncate">{{ item.room }}</div>
        </div>
    </div>
</template>
