<template>
    <div class="entity-list">
        <div class="row fw-bold border-bottom pb-2">
            ...
            <div class="col-auto d-none d-md-block">{{ $t("status") }}</div>
            <div class="col-auto d-none d-md-block">{{ $t("totalAmount") }}</div>
        </div>
        <div v-for="item in items" :key="item.$id" class="row border-bottom py-2" :class="{ 'is-selected': isSelected(item) }">
            ...
            <div class="col-auto d-none d-md-block">
                <StatusBadge :status="item.status" :variants="invoiceStatusVariants" :label="$t(`invoiceStatus.${item.status}`)" />
            </div>
            <div class="col-auto d-none d-md-block">{{ formatCurrency(item.totalAmount, undefined, "EUR") }}</div>
        </div>
    </div>
</template>
...
import { invoiceStatusVariants } from "@/components/status/variants"
