<template>
    <div class="row border-bottom py-2 align-items-center">
        ...
        <div class="col d-none d-md-block text-truncate">{{ $t(`vehicleType.${item.type}`) }}</div>
        <div class="col d-none d-lg-block"><StatusBadge :status="item.status" :variants="vehicleStatusVariants" :label="$t(`vehicleStatus.${item.status}`)" /></div>
        <div class="col-2 d-none d-xl-block text-truncate">{{ formatNumber(item.mileage) }} km</div>
...
import { formatNumber } from "@regira/modules/vue/formatters"
import type { SaveResult } from "@regira/modules/vue/entities"
import config from "../config/config"
import Entity from "../data/Entity"
import FormModalButton from "../details/FormModalButton.vue"
import StatusBadge from "@/components/status/StatusBadge.vue"
import { vehicleStatusVariants } from "@/components/status/variants"
