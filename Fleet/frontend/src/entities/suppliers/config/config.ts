...
const config: IConfig = {
    id: Entity.name,
    key: "Supplier",
    isComplex: true, // owns a collection (supported intervention types) -> Details page

    routePrefix: "suppliers",
    baseQueryParams: { includes: ["All"] }, // SupportedInterventionTypes is a flag-gated collection (untyped EntityIncludes -> only Default/All)
    ...
    overviewTitle: "suppliers", // camelCase i18n keys (multi-word → e.g. shoppingLists / shoppingList) — add matching entries to public/data/translations.json, or the nav renders the raw key
