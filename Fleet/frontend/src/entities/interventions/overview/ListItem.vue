<template>
    <div class="row border-bottom py-2 align-items-center">
        ...
        <div class="col text-truncate">
            <VehicleButton :model-value="getVehicle(item.vehicle)" /> {{ getVehicle(item.vehicle)?.licensePlate }}
            <Icon v-if="item.invoiceId" name="bi bi-receipt" :title="$t('invoiced')" class="text-muted ms-1" />
        </div>
        ...
        <div class="col d-none d-md-block">
            <StatusBadge :status="item.status" :variants="interventionStatusVariants" :label="$t(`interventionStatus.${item.status}`)" />
        </div>
        <div class="col d-none d-xl-block text-truncate">{{ formatDate(item.scheduledDate) }}</div>
        <div class="col-2 text-truncate text-end">{{ formatCurrency(item.cost, undefined, "EUR") }}</div>
