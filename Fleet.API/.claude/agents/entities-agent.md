...
Before writing any code, load the Entities guides using **one** of the following paths:

**If the Regira MCP server is configured** (preferred — no build required):
```
get_package("Regira.Entities", "instructions")
get_package("Regira.Entities", "signatures")
get_package("Regira.Entities", "namespaces")
```

**Otherwise**, read these three files in full from `.github/instructions/`:
