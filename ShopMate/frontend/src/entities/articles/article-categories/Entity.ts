...
// A join row linking Article to Category (back-end `e.Related(x => x.Categories)`). Rows are picked
...
// the full model class instead is what makes `add({ categoryId, category })` fail to type-check.
//
