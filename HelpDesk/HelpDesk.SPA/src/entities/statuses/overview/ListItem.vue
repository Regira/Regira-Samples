<template>
    <div class="row border-bottom py-2">
        ...
        <div class="col text-truncate">
            <span v-if="item.colorHex" class="badge" :style="{ backgroundColor: item.colorHex }">{{ item.$title }}</span>
            <template v-else>{{ item.$title }}</template>
        </div>
        <div class="col d-none d-md-block text-truncate">{{ item.sortOrder }}</div>
        <div class="col d-none d-lg-block text-truncate">
            <Icon v-if="item.isClosed" name="selected" />
        </div>
