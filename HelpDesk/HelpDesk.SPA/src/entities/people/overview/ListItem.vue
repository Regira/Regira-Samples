<template>
    <div class="row border-bottom py-2">
        ...
        <div class="col text-truncate" :class="{ 'text-muted': !item.isActive }">
            {{ item.$title }} <span v-if="!item.isActive" class="badge text-bg-secondary">{{ $t("inactive") }}</span>
        </div>
        <div class="col d-none d-md-block text-truncate">{{ item.email }}</div>
        <div class="col-1 d-none d-lg-block text-truncate">{{ $t(item.role) }}</div>
        <div class="col d-none d-lg-block text-truncate">
            <template v-if="item.supportTeam">
