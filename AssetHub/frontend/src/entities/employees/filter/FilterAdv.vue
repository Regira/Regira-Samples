<template>
    <div class="adv-filter">
        ...
        <input v-model.lazy.trim="searchObject.name" class="form-control mb-2" :placeholder="$t('name')" @change="handleUpdate" />
        <input v-model.lazy.trim="searchObject.department" class="form-control mb-2" :placeholder="$t('employee.department')" @change="handleUpdate" />
        <div class="mb-2">
            <NullableCheckBox v-model="searchObject.isActive" id="isActiveFilter" :label="$t('employee.isActive')" @update:modelValue="handleUpdate" />
        </div>
    </div>
</template>
