<template>
    <div class="entity-list">
        <div class="row fw-bold border-bottom pb-2">
            ...
            <div class="col d-none d-md-block">{{ $t("floor") }}</div>
            <div class="col-2">{{ $t("capacity") }}</div>
        </div>
        <div v-for="item in items" :key="item.$id" class="row border-bottom py-2" :class="{ 'is-selected': isSelected(item) }">
            ...
            <div class="col d-none d-md-block text-truncate">{{ item.floor?.building?.title }} &middot; {{ item.floor?.title }}</div>
            <div class="col-2">{{ item.capacity }}</div>
        </div>
    </div>
</template>
