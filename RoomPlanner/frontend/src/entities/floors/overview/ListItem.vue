<template>
    <div class="row border-bottom py-2">
        ...
        <div class="col d-none d-md-block text-truncate">
            <BuildingButton :model-value="getBuilding(item.building)" /> {{ getBuilding(item.building)?.$title }}
        </div>
        <div class="col-2 d-none d-lg-block text-truncate">{{ item.level }}</div>
...
import { FormModalButton as BuildingButton, useEntityStore as useBuildingStore } from "@/entities/buildings"
...
const { fromPool: getBuilding } = useBuildingStore()
</script>
