...
- Call `services.UseRegira(configuration["Regira:LicenseKey"])` **before** `UseEntities()` — optional but recommended for paid limits. Without a key the free tier applies automatically (5 simple / 2 complex entity registrations; product code `regira.entities`). Multiple product keys are supported: call `UseRegira()` once per key; the system picks the best license per product (paid always wins over free). Obtain a key at [https://regira.com/licensing](https://regira.com/licensing).
...
public static IServiceCollection AddEntityServices(this IServiceCollection services, IConfiguration configuration)
    => services
        .UseEntities<AppDbContext>(options =>
        {
            ...
            // License is resolved from the UseRegira() registration — no LicenseKey here
            options.UseDefaults();
...
> **First-build usings quick-reference** — the four most commonly missed extension-method namespaces:
>
> | `using` | Enables |
> |---------|---------|
> | `Regira.Entities.Mapping.Mapster` | `options.UseMapsterMapping()` |
> | `Regira.Entities.EFcore.Primers` | `options.AddPrimerInterceptors(sp)` in `AddDbContext` |
> | `Regira.Entities.EFcore.Normalizing` | `options.AddNormalizerInterceptors(sp)` in `AddDbContext` |
> | `Regira.DAL.EFcore.Services` | `options.AddAutoTruncateInterceptors()` in `AddDbContext` |
>
