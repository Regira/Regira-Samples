...
`Regira.Entities.DependencyInjection` validates a license key at startup (product code `regira.entities`). Register the license **once** via `UseRegira()` before calling `UseEntities()`:

```csharp
// Program.cs — before UseEntities
services.UseRegira(configuration["Regira:LicenseKey"]);

services.UseEntities<AppDbContext>(options =>
{
    // no LicenseKey here — resolved from the registered License
    options.UseDefaults();
});
```

Without a key, a **free tier** applies automatically (5 simple / 2 complex entity registrations) — no configuration required for dev/CI. Store a paid key under `Regira:LicenseKey` in `appsettings.json`. Multiple product keys are supported — call `UseRegira()` once per key; the system picks the best per product (paid always wins over free). Obtain a key at [https://regira.com/licensing](https://regira.com/licensing).
...
**Use `EntityControllerBase`** for exposing entity operations as HTTP endpoints (→ see Step 13 for the full pairing table).
...
- Use `PUT /{id}` for full replacements and `PATCH /{id}` for partial updates (JSON Merge Patch)
...
> **`IEntityServiceCollection<T>` vs `EntityServiceCollection<T>`:** `UseEntities<TContext>()` returns the concrete `EntityServiceCollection<TContext>`, which implements `IEntityServiceCollection<TContext>`. Always write your extension methods against the **interface** (`this IEntityServiceCollection<TContext> e`), not the concrete class — this keeps the API surface consistent and avoids binding to implementation details.

> **`EntityServiceCollection<TContext>` implements `IServiceCollection`** — it inherits from `ServiceCollectionWrapper : IServiceCollection`, so returning it from a method typed `IServiceCollection` compiles directly (no `.GetServices()` call needed at the end of the chain). You can also call `.GetServices<TContext>()` to explicitly unwrap to `IServiceCollection` if needed, e.g. when the chain is broken across multiple statements.
...
| 8. Preppers | Skip by default | You must compute totals/codes/FKs or manage child collections before the entity reaches EF Core |
...
**Mnemonic:** Preppers run synchronously *inside* the `Add()` / `Modify()` / `Save()` service-layer calls, before the entity reaches EF Core's change tracker. When seeding, computed properties (totals, codes, FKs) are available immediately after `await service.Add(item)` returns — you do not need to call `SaveChanges()` first to access them. **Exception — database-assigned IDs:** `Id` on `IEntityWithSerial` (auto-increment int) is assigned by the database during `SaveChanges()`, not during `Add()`. When seeding chains depend on a parent's auto-generated `Id` (e.g. `child.ParentId = parent.Id`), call `await parentService.SaveChanges()` after adding the parent batch so `Id` is populated before assigning it to children. Primers run later, inside EF Core's `SaveChanges` interceptor — they can inspect other modified entities in the same transaction.
...
> ⚠️ **Rule of thumb:** use `Related()` only when the child entity has no `IEntityService<T>` of its own. Using it on an independently-managed entity will silently overwrite that service's state.
...
All three interceptor registration calls use the `(sp, options) =>` factory overload of `AddDbContext`. Register only the ones you need.

| Method | What it does | When to add |
|---|---|---|
| `AddPrimerInterceptors(sp)` | Runs Primers during EF Core `SaveChanges` (timestamps, soft-delete, audit, custom primers) | When any `IEntityPrimer` is registered (always required with `UseDefaults()`) |
| `AddNormalizerInterceptors(sp)` | Runs entity normalizers during `SaveChanges` to populate `NormalizedContent` and other `[Normalized]` fields | When any entity uses `[Normalized]` or normalizer services |
| `AddAutoTruncateInterceptors()` | Silently truncates `string` values to their `[MaxLength]` before `SaveChanges` to prevent DB exceptions | When entities declare `[MaxLength]` and silent truncation is acceptable |
...
> **Nullability convention:** `IHasCode`, `IHasTitle`, `IHasDescription`, and `IHasNormalizedContent` all declare their string properties as `string?`. This is a contract convention — the interface does not enforce a value. If your domain requires a non-null value, add `[Required]` to the implementing property; the `string?` type declaration remains unchanged. Use the auto-truncate and normaliser interceptors to handle whitespace and casing.
