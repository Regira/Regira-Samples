<template>
    <div class="row border-bottom py-2">
        ...
        <div class="col text-truncate">
            {{ item.$title }}
            <span v-if="!item.isActive" class="badge bg-secondary ms-1">{{ $t("inactive") }}</span>
        </div>
        <div class="col d-none d-md-block text-truncate">{{ item.department }}</div>
        <div class="col d-none d-lg-block text-truncate">
            <span class="badge" :class="item.role === 'Admin' ? 'bg-primary' : 'bg-light text-dark'">{{ $t(item.role.toLowerCase()) }}</span>
        </div>
        <div class="col-2 d-none d-xl-block text-truncate">
            <Icon :name="item.isActive ? 'selected' : 'clear'" :class="item.isActive ? 'text-success' : 'text-muted'" />
        </div>
